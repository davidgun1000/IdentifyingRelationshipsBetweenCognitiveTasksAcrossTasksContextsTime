...
load('LBA_Forstmann_scanner.mat'); %load the dataset, see an example in the 'LBA_Forstmann_scanner.mat' 
...
num_randeffect=14; %the total number of random effects in the LBA model. For Forstmann dataset, we have 7 random effects.
...
param.theta_mu=[0.201;0.249;-0.069;-0.373;0.091;1.058;-1.761;0.412;0.507;0.120;-0.171;0.371;1.150;-1.670]; %the initial values for parameter \mu
...
param.num_randeffect=14; %the total number of random effects in the LBA model. For Forstmann dataset, we have 7 random effects.

num_choice=2;% the number of choice
burn=500;% the burn in iterations
nit=12000; % we take the last 10000 draws out of 12000 draws
s=100000;% the maximum total number of iterations

[theta_latent]=LBA_MC_v1(data,param,num_subjects,num_trials,num_particles); %obtain initial values of the random effects.

tot_param=133; %total number of parameters for each subjects
...
while i<=s
   ...
   if sum(count>switch_num)==num_subjects
         for j=1:num_subjects 
             theta=[theta_latent_b10_store(burn:end,j),theta_latent_b20_store(burn:end,j),theta_latent_b30_store(burn:end,j),theta_latent_A0_store(burn:end,j),...
                 theta_latent_v10_store(burn:end,j),theta_latent_v20_store(burn:end,j),theta_latent_tau0_store(burn:end,j),...
                 theta_latent_b11_store(burn:end,j),theta_latent_b21_store(burn:end,j),theta_latent_b31_store(burn:end,j),theta_latent_A1_store(burn:end,j),...
                 theta_latent_v11_store(burn:end,j),theta_latent_v21_store(burn:end,j),theta_latent_tau1_store(burn:end,j),...
                 ...
                 chol_theta_sig2_store6(burn:end,:),chol_theta_sig2_store7(burn:end,:),chol_theta_sig2_store8(burn:end,:),...
                 chol_theta_sig2_store9(burn:end,:),chol_theta_sig2_store10(burn:end,:),chol_theta_sig2_store11(burn:end,:),...
                 chol_theta_sig2_store12(burn:end,:),chol_theta_sig2_store13(burn:end,:),chol_theta_sig2_store14(burn:end,:)];
   ...
   [theta_latent]=LBA_CMC_v1(data,param,theta_latent,num_subjects,num_trials,num_particles,mean_theta,covmat_theta,i,count,switch_num);
    ...
    theta_latent_v20_store(i,:)=theta_latent(:,6)';
    theta_latent_tau0_store(i,:)=theta_latent(:,7)';
    
    theta_latent_b11_store(i,:)=theta_latent(:,8)';
    theta_latent_b21_store(i,:)=theta_latent(:,9)';
    theta_latent_b31_store(i,:)=theta_latent(:,10)';
    theta_latent_A1_store(i,:)=theta_latent(:,11)';
    theta_latent_v11_store(i,:)=theta_latent(:,12)';
    theta_latent_v21_store(i,:)=theta_latent(:,13)';
    theta_latent_tau1_store(i,:)=theta_latent(:,14)';
    ...
    %save the output to your directory
     if i==500 | i==1000 | i==2000 | i==3000 | i==4000 | i==5000 | i==6000 | i==7000 | i==8000 | i==9000 | i==10000 | i==20000 | i==30000 | i==40000 | i==50000 | i==60000 | i==70000 | i==80000 | i==90000 | i==100000
        save('/srv/scratch/z3512791/LBA_Forstmann_covariance.mat','theta_mu_store','theta_sig2_store1','theta_sig2_store2','theta_sig2_store3','theta_sig2_store4',...
             ...
             'theta_sig2_store13','theta_sig2_store14',...
             ...
             'theta_latent_b10_store','theta_latent_b20_store','theta_latent_b30_store','theta_latent_A0_store','theta_latent_v10_store','theta_latent_v20_store','theta_latent_tau0_store',...
             'theta_latent_b11_store','theta_latent_b21_store','theta_latent_b31_store','theta_latent_A1_store','theta_latent_v11_store','theta_latent_v21_store','theta_latent_tau1_store'); 
     end
     i=i+1;   
end
...
theta_latent_v20_store=theta_latent_v20_store(length_draws-9999:end,:);
...
theta_latent_v21_store=theta_latent_v21_store(length_draws-9999:end,:);
...
% save('LBA_Forstmann_covariance.mat','theta_mu_store','theta_sig2_store1','theta_sig2_store2','theta_sig2_store3','theta_sig2_store4',...
%              'theta_sig2_store5','theta_sig2_store6','theta_sig2_store7','theta_sig2_store8',...
%              'theta_sig2_store9','theta_sig2_store10','theta_sig2_store11','theta_sig2_store12',...
%              'theta_sig2_store13','theta_sig2_store14',...
%              'chol_theta_sig2_store1','chol_theta_sig2_store2','chol_theta_sig2_store3',...
